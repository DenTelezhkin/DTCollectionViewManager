...
test-iOS:
	set -o pipefail && xcodebuild -project DTCollectionViewManager.xcodeproj -scheme DTCollectionViewManager-iOS -destination "name=iPhone 6s" -enableCodeCoverage YES test | xcpretty -ct
	bash <(curl -s https://codecov.io/bash)

test-tvOS:
	set -o pipefail && xcodebuild -project DTCollectionViewManager.xcodeproj -scheme DTCollectionViewManager-tvOS -destination "name=Apple TV 1080p" -enableCodeCoverage YES test | xcpretty -ct
	bash <(curl -s https://codecov.io/bash)
